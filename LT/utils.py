...
def save_checkpoint(args, state, is_best, encoder=True):
    if encoder:
        filename = os.path.join(args.store_name, "cl_ckpt.pth.tar")
    else:
        filename = os.path.join(args.store_name, "cl_cls_ckpt.pth.tar")
    ...
    if is_best:
        if encoder:
            ...
            shutil.copyfile(
                filename, filename.replace("cl_cls_ckpt.pth.tar", "best_cls.pth.tar")
            )
