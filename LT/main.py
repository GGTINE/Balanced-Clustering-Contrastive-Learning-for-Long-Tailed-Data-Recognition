...
def main_worker(args):
    ...
    # criterion_cs = nn.CosineSimilarity().cuda()
    ...
    if args.eval:
        acc1, many, med, few = evaluate(
            train_loader, test_loader, model, criterion_ce, 1, args, tf_writer
        )
        print(
            "Shot: Prec@1: {:.3f}, Many Prec@1: {:.3f}, Med Prec@1: {:.3f}, Few Prec@1: {:.3f}".format(
                acc1, many, med, few
            )
        )
        return
...
def train(
    ...
    # feature_sum = torch.zeros_like(feature_class_average)
    ...
    for i, data in enumerate(pbar):
        ...
        with autocast(enabled=True):
            ...
            ce_loss = criterion_ce(
                logits, targets, args.epochs, epoch
            )
            # compute loss
            if args.warmup_epochs > epoch:
                ...
                if epoch % 10 == 0:
                    for k in range(num_classes):
                        # centroids = kmeans.fit(feature_list[k].detach().cpu().numpy().reshape(-1, args.feat_dim))
                        # feature_average[k] = torch.tensor(centroids.cluster_centers_)
                        centroids = kmeans(torch.unsqueeze(feature_list[k], 0))
                        feature_average[k] = torch.squeeze(centroids)
                        # feature_average[k] = torch.tensor(centroids.cluster_centers_).repeat(5, 1)[:args.centroid]
                # _, centroids = kmeans.fit_predict(feature_list[k])
                # feature_average[k] = centroids
        ...
        with torch.no_grad():
            ...
            # count_labels = count_labels + torch.bincount(
            #     average_target, minlength=num_classes
            # )
    ...
    # feature_average = feature_sum / torch.where(
    #     count_labels == 0, 1, count_labels
    # ).unsqueeze(1)
    # feature_average = centroids.detach().cpu()
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        if args.only_decoupling:
            tf_writer.add_scalar("DE loss/CE_val", ce_loss_all.avg, epoch)
            tf_writer.add_scalar("DE acc/val_top1", top1.avg, epoch)
        else:
